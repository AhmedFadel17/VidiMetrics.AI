        ...
        <section className="py-32 px-8">
          <div className="max-w-screen-2xl mx-auto">
            <div className="text-center mb-20 space-y-4">
              <h2 className="text-xs uppercase tracking-widest text-tertiary font-bold">The Creative Investment</h2>
              <h3 className="text-4xl md:text-6xl font-headline font-bold text-on-surface">Choose Your Level.</h3>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                { name: 'Free', price: '0', desc: 'For new observers.', features: ['5 AI Storyboards / mo', 'Basic Analytics', 'Standard Resolution'] },
                { name: 'Creator', price: '29', desc: 'For the storyteller.', featured: true, features: ['Unlimited Characters', 'Custom Prompt Tuning', 'High-res Downloads', 'Retention Heatmaps'] },
                { name: 'Studio', price: '99', desc: 'For the production team.', features: ['Team Collaboration', 'API Access', '4K Cinematic Export', 'Priority Rendering'] }
              ].map((tier) => (
                <div 
                  key={tier.name}
                  className={`p-10 rounded-3xl flex flex-col h-full transform transition-all hover:-translate-y-2 ${tier.featured ? 'bg-surface-container-high border-2 border-primary/30 shadow-[0_20px_50px_rgba(221,183,255,0.15)] relative' : 'bg-surface-container-low ghost-border'}`}
                >
                  {tier.featured && (
                    <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-primary text-on-primary px-4 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest">Most Popular</div>
                  )}
                  <div className="mb-8">
                    <h4 className={`text-xl font-headline font-bold mb-2 ${tier.featured ? 'text-primary' : 'text-on-surface'}`}>{tier.name}</h4>
                    <p className="text-on-surface-variant text-sm">{tier.desc}</p>
                  </div>
                  <div className="text-4xl font-headline font-bold text-on-surface mb-8">
                    ${tier.price}<span className="text-lg text-on-surface-variant font-normal">/mo</span>
                  </div>
                  <ul className="space-y-4 mb-10 flex-grow">
                    {tier.features.map((f) => (
                      <li key={f} className="flex items-center gap-3 text-sm text-on-surface-variant">
                        <span className={`material-symbols-outlined text-sm ${tier.featured ? 'text-primary' : 'text-on-surface-variant'}`}>check</span>
                        {f}
                      </li>
                    ))}
                  </ul>
                  <button className={`w-full py-4 rounded-xl font-headline font-bold transition-all ${tier.featured ? 'btn-gradient shadow-lg shadow-primary/20' : 'glass-morphism ghost-border hover:bg-surface-variant'}`}>
                    {tier.name === 'Studio' ? 'Contact Sales' : tier.name === 'Free' ? 'Start Free' : 'Join Creator'}
                  </button>
                </div>
              ))}
            </div>
