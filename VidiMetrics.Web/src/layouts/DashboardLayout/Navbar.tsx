                ...
                <div className="flex items-center gap-4 border-r border-white/5 pr-4">
                    ...
                    <Menu as="div" className="relative">
                        <Menu.Button className="w-10 h-10 flex items-center justify-center relative hover:bg-white/5 rounded-xl transition-colors group">
                            <span className="material-symbols-outlined text-white/60 text-xl group-hover:text-white transition-colors">notifications</span>
                            {hasNotifications && (
                            ...
                            <Menu.Items className="absolute right-0 bg-surface-container-low w-80 origin-top-right glass-card rounded-lg overflow-hidden focus:outline-none z-50 ring-1 ring-white/10">
                                <div className="p-4 border-b border-white/5 bg-white/5">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className="bg-yellow-400/20 text-yellow-400 text-[10px] px-2 py-0.5 rounded-full font-bold">NEW</span>
                                    </div>
                                </div>
                                <div className="max-h-96 overflow-y-auto">
                                    <div className="p-8 text-center flex flex-col items-center gap-3">
                                        <div className="w-12 h-12 rounded-full bg-white/5 flex items-center justify-center">
                                            <span className="material-symbols-outlined text-white/20 text-2xl">notifications_off</span>
                                        </div>
                                ...
                                <div className="p-3 bg-white/5 text-center border-t border-white/5">
                                    <button onClick={() => navigate("/dashboard/notifications")} className="text-[10px] uppercase tracking-widest font-bold text-primary hover:text-primary-light transition-colors">
                                        View All Activity
                                    </button>
                                </div>
                            </Menu.Items>
                    ...
                    <button className="w-10 h-10 flex items-center justify-center hover:bg-white/5 rounded-xl transition-colors group">
                        <span className="material-symbols-outlined text-white/60 text-xl group-hover:text-white transition-colors">language</span>
                    </button>
                </div>
                ...
                <Menu as="div" className="relative">
                    <Menu.Button className="flex items-center gap-4 p-1.5 outline-none">
                        ...
                        <div className="w-11 h-11 rounded-full overflow-hidden border-2 border-white/10 p-[2px] group-hover:border-primary/50 transition-all duration-300">
                            <img
                                src={user?.profile.picture || "https://api.dicebear.com/7.x/avataaars/svg?seed=Felix"}
                                alt="Operator"
                        ...
                        <Menu.Items className="absolute right-0 w-64 p-2 font-medium text-sm origin-top-right glass-card rounded-lg overflow-hidden focus:outline-none z-50 ring-1 ring-white/10 bg-surface-container-low">
                            <div className="p-3 border-b border-white/5">
                                <div className="flex flex-col">
                                    <span className="text-base capitalize text-white truncate">{user?.profile.given_name + ' ' + user?.profile.family_name}</span>
                                    <span className="text-sm text-white/40 truncate">{user?.profile.email}</span>
                                </div>
                            </div>
                            <div className=" py-3">
                                <Menu.Item>
