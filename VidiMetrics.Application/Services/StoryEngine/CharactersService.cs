...
public class CharactersService : ICharactersService
{
    ...
    public async Task<CharacterResponseDto> GetByIdAsync(Guid userId, Guid id, CancellationToken ct = default)
    {
        var entity = await _repository.Query()
            .Include(x => x.AiImage)
            .Include(x => x.VoiceProfile)
            .FirstOrDefaultAsync(s => s.Id == id && s.Show.UserId == userId, cancellationToken: ct);
        if (entity == null) throw new KeyNotFoundException("Character not found.");
        return _mapper.Map<CharacterResponseDto>(entity);
    }

    public async Task<PaginationResponseDto<CharacterResponseDto>> GetAllAsync(Guid userId, CharacterFilterDto filter, CancellationToken ct = default)
    {
        IQueryable<Character> query = _repository.Query().Include(x => x.AiImage).Include(x => x.VoiceProfile);
    ...
    {
        IQueryable<Character> query = _repository.Query().Include(x => x.AiImage);
        query = query.Where(x => x.Show.UserId == userId);
        if (showId.HasValue)
        {
    ...
    public async Task<CharacterResponseDto> CreateAsync(Guid userId, CreateCharacterDto dto)
    {
    ...
    public async Task<CharacterResponseDto> UpdateAsync(Guid userId, Guid id, UpdateCharacterDto dto)
    {
